...
public:
	...
	//constructorul din clasele de baza ce contin metode virtuale
	//va fi marcat ca virtual pentru a evita memory leaks
	virtual ~student()
	{
		if (note != nullptr)
		{
...
public:
	...
	~voluntar()
	{
		if (oreZilnice != nullptr)
		{
			delete[] oreZilnice;
		}
	}
	voluntar(const voluntar& c)
	{
		if (c.nrZile > 0 && c.oreZilnice != nullptr)
		{
			nrZile = c.nrZile;
			oreZilnice = new int[nrZile];
			for (int i = 0; i < nrZile; i++)
			{
				oreZilnice[i] = c.oreZilnice[i];
			}

		}
	}
	voluntar& operator=(const voluntar& c)
	{
...
public:
	...
	student_voluntar(string nume, const char* adresa, int nrZile, int* oreZilnice, const char* organizatie) :
		voluntar(nrZile, oreZilnice), student(nume, adresa)
	{
		if (organizatie != nullptr)
		{
			this->organizatie = new char[strlen(organizatie) + 1];
	...
	void metoda()
	{
		//cout << "Student voluntar" << endl;
		student::metoda();
		voluntar::metoda();
	}

	//supradefinire
	float punctajConcurs() override
	{
		float medie = student::punctajConcurs();
		if (medie > 9) {
			return 10;
		}
		else
...
int main()
{
	...
	voluntar v3 = v2;
	v1 = v2;
	student_voluntar sv7("Daniel", "Bucuresti, Sector 2", 3, ore, "Crucea Rosie");
	student_voluntar sv8 = sv7;
	sv6 = sv7;
	cout << endl;
	sv7.student::metoda();
	...
	cout << s4.punctajConcurs() << endl;

	sv6.setNote(note, 3);
	cout << sv6.punctajConcurs() << endl;

	s4 = sv6;
	cout << s4.punctajConcurs() << endl;

	//virtualizarea (supradefinirea) are loc doar la nivelul adreselor
	student* pstud = &sv6;
	cout << pstud->punctajConcurs() << endl;

	student** grupa = new student * [2];
	grupa[0] = new student("Gigel", note, 3);
	grupa[1] = new student_voluntar("Ionel", note, 3, "SiSC");
	for (int i = 0; i < 2; i++) {
		cout << grupa[i]->punctajConcurs() << " ";
	}
	for (int i = 0; i < 2; i++) {
		delete grupa[i];
	}
